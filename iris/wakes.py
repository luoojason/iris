...
KINDS = ("file_exists", "file_gone", "file_changed", "log_pattern")
    ...
    for index, rule in enumerate(rules):
        ...
        if kind == "log_pattern":
            ...
            if not isinstance(pattern, str) or not pattern:
                problems.append(f"{label}: log_pattern needs a regex pattern")
            else:
                try:
                    re.compile(pattern)
...
def _evaluate(rule: dict, entry: dict) -> tuple[bool, str]:
    """Evaluate one rule against its state entry. Returns (fired, detail).

    Mutates ``entry``'s observation fields (present/mtime/size/offset) so the
    next tick sees this tick's world. stat/read calls only — nothing here may
    ever call the model or the network.
    """
    kind = rule["kind"]
    path = Path(rule["path"])
